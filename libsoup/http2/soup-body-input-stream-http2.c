                                        ...
                                        GError       **error)
{
        ...
        priv->pos += count - rest;
        if (priv->session)
                NGCHECK (nghttp2_session_consume (priv->session, priv->stream_id, count - rest));
        ...
        if (!priv->completed) {
                ...
                if (read_error && !g_error_matches (read_error, G_IO_ERROR, G_IO_ERROR_WOULD_BLOCK)) {
        ...
        if (blocking && rest && (!priv->completed || priv->pos < priv->len)) {
                return count + soup_body_input_stream_http2_read_real (
                        stream, blocking, (guint8*)buffer + count, rest, cancellable, error
                );
        }
