				  ...
				  GError         **error)
{
	...
	/* NOTE: all error domains/quarks are designed to match GZlibDecompressor even if GConverter has its own */
	...
	*bytes_written = outbuf_size - available_out;
	...
	if (result == BROTLI_DECODER_RESULT_SUCCESS)
		...
		g_set_error_literal (error, G_IO_ERROR, G_IO_ERROR_PARTIAL_INPUT, "SoupBrotliDecompressorError 2: More input required (corrupt input)");
		return G_CONVERTER_ERROR;
	}
